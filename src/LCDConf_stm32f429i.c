...
#define NUM_VSCREENS 1 // Number of virtual screens to be used. See VSCREEN_*.c exmaples.

#define BK_COLOR GUI_WHITE

#undef  GUI_NUM_LAYERS
#define GUI_NUM_LAYERS 1 // TODO: Use layers (Chapter 24.3) for MultiDisplay support
...
// VRAM SDRAM (32 MB) 0xC0000000 to 0xC2000000
#define LCD_LAYER0_FRAME_BUFFER  ((uint32_t)0xC1000000)
#define LCD_LAYER1_FRAME_BUFFER  ((uint32_t)0xC1500000)


static int H_SYNC = 0;
static int V_SYNC = 0;
static int H_BACK_P = 0;
static int V_BACK_P = 0;
static int H_FRONT_P = 0;
static int V_FRONT_P = 0;
static int H_ACTIVE = 0;
...
// TODO: Optimal sizing of these buffers
...
static void 	set_screen_calibration_values();
...
void HAL_LTDC_LineEvenCallback(LTDC_HandleTypeDef *hltdc)
{
  ...
  for (layer = 0; layer < GUI_NUM_LAYERS; layer++) 
  {
    if (layer_prop[layer].pending_buffer >= 0) 
    {
      ...
      HAL_LTDC_SetAddress(hltdc, Addr, layer); // TODO: Pending buffer not used, so it shows garbage
...
//  printf("LCD_X_DisplayDriver(Cmd == %u)\n\r", Cmd);
#endif
  int32_t r = 0;
  uint32_t addr;
  int32_t xPos, yPos;
  ...
  case LCD_X_SETORG: 
	  // TODO: this addr calculation looks suspect. perhaps it is: address + (yPos*xSize + xPos)*BytesPerPixel
...
static void set_screen_calibration_values() {
	int is_logics_display = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_12);

	if (is_logics_display) {
		H_SYNC = 1;
		V_SYNC = 1;
		H_BACK_P = 46;
		V_BACK_P = 23;
...
static void LCD_LL_Init(void) 
{
  ...
  */
  ...
  set_screen_calibration_values();
      
  /* Set LCD Timings */
  hltdc.Init.HorizontalSync = H_SYNC;
  hltdc.Init.VerticalSync = V_SYNC;
  hltdc.Init.AccumulatedHBP = H_BACK_P;
  hltdc.Init.AccumulatedVBP = V_BACK_P;

  // ST indicate this is correct..but there is artefact across btm of screen  
  //hltdc.Init.AccumulatedActiveH = V_ACTIVE; //+ V_SYNC + V_BACK_P - 1;
  //hltdc.Init.AccumulatedActiveW = H_ACTIVE; //+ H_SYNC + H_BACK_P -1 ;
  // but other examples use..
  hltdc.Init.AccumulatedActiveH = V_ACTIVE + V_SYNC + V_BACK_P;
  hltdc.Init.AccumulatedActiveW = H_ACTIVE + H_SYNC + H_BACK_P;
  
  hltdc.Init.TotalHeigh = V_ACTIVE + V_SYNC + V_BACK_P + V_FRONT_P;
  hltdc.Init.TotalWidth = H_ACTIVE + H_SYNC + H_BACK_P + H_FRONT_P;
  ...
  PeriphClkInitStruct.PLLSAI.PLLSAIN = 192; //192;
  ...
  hdma2d.Init.Mode         = DMA2D_M2M_PFC; // DMA2D_R2M;
...
/**
  ...
  * @retval None
  */
static void DMA2D_DrawBitmapL8(void * pSrc, void * pDst,  int32_t OffSrc, int32_t OffDst, uint32_t PixelFormatDst, uint32_t xSize, uint32_t ySize)
{
  /* Set up mode */
  DMA2D->CR      = 0x00010000UL | (1 << 9);         /* Control Register (Memory to memory with pixel format conversion and TCIE) */
